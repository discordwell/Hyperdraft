...
### Shock lands (resolved by Agent O — `make_shockland_setup` framework)

All 10 Ravnica-style "As this land enters, you may pay 2 life. If you don't,
it enters tapped." lands now wire through ``make_shockland_setup(life_cost=2)``
in ``src/cards/interceptor_helpers.py``. The framework opens a
``PendingChoice`` (``choice_type='shockland'``) on ETB owned by the
controller; AI players auto-resolve via the heuristic
``life > life_cost + 3`` while humans see the standard choice UI.

EOE cards (5): BREEDING_POOL, GODLESS_SHRINE, SACRED_FOUNDRY, STOMPING_GROUND,
WATERY_GRAVE.

ECL cards (5): BLOOD_CRYPT, HALLOWED_FOUNTAIN, OVERGROWN_TOMB, STEAM_VENTS,
TEMPLE_GARDEN.

Coverage: ``tests/test_phase5b_shockland.py`` (15 tests).
