...
### Saga chapter abilities not implemented (13 cards) — RESOLVED

> Resolved by the Saga framework (`src/engine/saga.py` + `make_saga_setup`).
> All ~21 sagas across WOE/SPM/TLA/DSK/FIN/TDM are now wired and tick
> through chapters with sacrifice-after-final semantics. Individual
> chapters whose effect still requires deeper engine work (target opp
> creature, etc.) emit a TARGET_REQUIRED event; chapters returning `[]`
> are intentional noops documented in the card setup.
...
### Sagas not implemented (3 cards) — RESOLVED via Saga framework
...
### Saga chapter triggers (2 cards) — RESOLVED via Saga framework
