...
- **2026-05-09 (P2b iter-3 double, vs FINA_high_frequency)**: **Lost T13**; P2=2, P1=9. P2 died at MARKET_CLOSE T13 from leverage tick (2-2=0). Pilot B report. Key findings:
  - **Gamma Scalper (Lev3) was the game-deciding mistake.** GS deployed T8 without TDT or TDC in hand or on board. Raised Σlev from 2 to 5. At 5 capital/turn drain with no counter-removal, the game was unrecoverable from T8. **Rule: GS is unplayable without BSM or TDT on board.** Encoder heuristic added.
  - **Block command broken from P2 seat (Bug 28 confirmed).** Every block attempt T5-T9 returned "not found on opponent's battlefield." FCB+TTD went unblocked for 5 turns. P2 cannot manually assign blocks in two-pilot mode — this is the primary structural disadvantage.
  - **No counter-removal drawn in 13 turns.** With 0 TDC or TDT across 7 opening + 6 drawn cards, counter-remover density in the deck may be too low. Target: 6-7 counter-removers in deck construction.
  - **HF vs Derivatives matchup confirmed ~65/35 HF favored** without counter-removal early. If TDT or TDC is in opening hand, matchup likely closer to 50/50.
  - **Leverage tick math confirmed accurate.** P2 capital dropped exactly by Σlev each MC: T4→T5: -1 (Σlev=1); T8→T9: -4 (Σlev=4 after GS). No 1.7× bug observed. The ×1.7 safety factor in `_filter_trap_cards` is now acknowledged as overly conservative.
  - Pilot B report: `logs/finance_ultra_iter3_double_pilotB.md`.

- **2026-05-09 (P2b iter-1 double, vs FINA_high_frequency)**: **Lost T10**; P2=−1, P1=10. P2 died at MARKET_CLOSE from Σlev=3 leverage tick. Note: P2's T2-T8 moves were contaminated by Bug 25 (no seat isolation; P1 orchestration played both seats in early turns). The strategic takeaways below hold regardless of contamination because the contaminated plays matched the canonical Derivatives line. Key findings:
  - **TDC and TDT must be top-1 priority T1-T4.** Neither was drawn this game. Without a counter-remover, Σlev=3 (UAR Lev1 + HF PM Lev2 + DH Lev2) = 3 self-damage/turn from T8 onward = lethal within 3 turns. Mulligan policy confirmed: **auto-keep any hand with TDT or TDC.**
  - **Never attack with all Traders when opponent has ≥3 bodies.** T8 ODI+UAR attack left zero blockers for T9; P1's 5-body wave was uncontested. Rule: keep highest-toughness Trader untapped when facing ≥3 opponent bodies.
  - **Σlev=3 tick (3/turn) is the actual loss vector.** P2 took ~22 combat damage in 10 turns (avg 2.2/turn) but died at MC tick, not from combat. UAR (Lev1) + HF PM (Lev2) = 3/turn from T8. The tick is not avoidable without counter-removers.
  - **HF vs Derivatives matchup: HF ~65/35 favored without Collar.** Confirmed by both pilots. Matchup may flip when TDC+TDT are deployed T2-T3 and Σlev stays ≤2.
  - **BSM activation status unclear.** BSM was deployed T6 but no confirmed activation observed. If BSM never activated, it contributed 0 counter-removal. Investigate whether BSM requires an explicit paid activation the AI never triggers.
  - Pilot report: `logs/finance_ultra_iter1_double_pilotB.md`.
