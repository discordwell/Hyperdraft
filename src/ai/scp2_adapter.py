...
best move, fires the real engine verb (scp2.play_card / advance / contain / infiltrate /
...
SUPPORTED_SCP2_DIFFICULTIES = frozenset({"easy", "medium", "hard"})


def validate_scp2_difficulty(difficulty: str) -> str:
    norm = str(difficulty).strip().lower()
    if norm not in SUPPORTED_SCP2_DIFFICULTIES:
        supported = ", ".join(sorted(SUPPORTED_SCP2_DIFFICULTIES))
        raise ValueError(f"Unknown scp2 difficulty: {difficulty!r}. Supported: {supported}")
    return norm


# --------------------------------------------------------------------------- readers
def _hand_objs(state: GameState, pid: str) -> list:
    return [state.objects[o] for o in scp2.hand_ids(state, pid) if o in state.objects]


def _of_kind(objs: list, kind: CardType) -> list:
    return [o for o in objs if getattr(o.card_def, "scp2_kind", None) == kind]


def _cost(obj) -> int:
    return int(getattr(obj.card_def, "scp2_cost", 0) or 0)
...
def _installed(state: GameState, ids: list, role: str) -> list:
    ...
    for oid in ids:
        ...
        if o and getattr(o.state, "scp2_role", None) == role:
            out.append(o)
    return out
...
class SCP2AIAdapter:
    ...
    def __init__(self, difficulty: str = "medium"):
        self.difficulty = validate_scp2_difficulty(difficulty)

    async def take_turn(self, player_id: str, state: GameState, game) -> list:
        faction = scp2.faction_of(state, player_id)
        chooser = (self._foundation_action if faction == scp2.FOUNDATION
                   else self._insurgency_action)
        events: list = []
        for _ in range(24):  # safety cap; the choosers always act while AP remains
            if game.is_game_over():
                break
            r = scp2.ensure_scp2_state(state, player_id)
            if r["ap"] <= 0:
                break
    ...
    def _foundation_action(self, game, pid, state, r):
        ...
        for cell in cells:
            ...
            if a and getattr(a.state, "scp2_status", None) == "advancing":
                advancing.append((a, cell))
        real_advancing = [(a, c) for (a, c) in advancing if int(getattr(a.card_def, "scp2_value", 0)) > 0]

        # 1. Contain any *real* anomaly that has met its threshold — never leave points stealable.
        for (a, cell) in real_advancing:
            thr = int(getattr(a.card_def, "scp2_threshold", 0))
            if int(getattr(a.state, "scp2_advancement", 0)) >= thr:
                ok, _m, evs = scp2.contain(game, pid, a.id)
                if ok:
                    return True, evs

        # 2. Stay solvent.
        if r["credits"] < 2:
            ok, _m, evs = scp2.gain_credits(game, pid)
            if ok:
                return True, evs

        # 3. Start a win-con if none is advancing.
        anomaly_cards = _of_kind(objs, CardType.SCP2_ANOMALY)
        real_anoms = [o for o in anomaly_cards if not getattr(o.card_def, "scp2_trap", False)]
        traps = [o for o in anomaly_cards if getattr(o.card_def, "scp2_trap", False)]
        if not real_advancing and real_anoms:
            card = min(real_anoms, key=lambda o: (_cost(o), int(getattr(o.card_def, "scp2_threshold", 0))))
            if _affordable(card, r):
                ok, _m, evs = scp2.play_card(game, pid, card.id)
                if ok:
                    return True, evs

        # 4. Defend an undefended advancing anomaly (hard/medium only).
        layer_cards = _of_kind(objs, CardType.SCP2_LAYER)
        if layer_cards and self.difficulty != "easy":
            for (a, cell) in (real_advancing + advancing):
                if len(cell["layers"]) < 1:
                    lc = max(layer_cards, key=lambda o: int(getattr(o.card_def, "scp2_strength", 0)))
                    if _affordable(lc, r):
                        ok, _m, evs = scp2.play_card(game, pid, lc.id, target=("cell", cell["id"]))
                        if ok:
                            return True, evs

        # 5. Lay an early economy engine.
        asset_cards = _of_kind(objs, CardType.SCP2_ASSET)
        if asset_cards and len(r["assets"]) < 2:
            ac = min(asset_cards, key=_cost)
            if _affordable(ac, r):
                ok, _m, evs = scp2.play_card(game, pid, ac.id)
                if ok:
                    return True, evs

        # 6. Advance the lead real anomaly toward its lock.
        if real_advancing and r["credits"] >= 1:
            lead = max(real_advancing, key=lambda ac: int(getattr(ac[0].state, "scp2_advancement", 0)))
            ok, _m, evs = scp2.advance(game, pid, lead[0].id)
            if ok:
                return True, evs
        ...
        if traps and 1 <= len(advancing) < 3:
            tc = traps[0]
            if _affordable(tc, r):
                ok, _m, evs = scp2.play_card(game, pid, tc.id)
                if ok:
                    return True, evs
        for (a, cell) in advancing:
            if int(getattr(a.card_def, "scp2_value", 0)) == 0 and int(getattr(a.state, "scp2_advancement", 0)) < 2 and r["credits"] >= 1:
                ok, _m, evs = scp2.advance(game, pid, a.id)
                if ok:
                    return True, evs
        ...
        if op is not None and _affordable(op, r):
            ok, _m, evs = scp2.play_card(game, pid, op.id)
            if ok:
                return True, evs
        ...
        if act_id:
            ok, _m, evs = scp2.activate_ability(game, pid, act_id)
            if ok:
                return True, evs

        # 9. Draw if thin, else bank Funding.
        if len(scp2.hand_ids(state, pid)) < 3:
            ok, _m, evs = scp2.draw_action(game, pid)
            if ok:
                return True, evs
        ok, _m, evs = scp2.gain_credits(game, pid)
    ...
    def _pick_foundation_operation(self, state, pid, objs, r):
        ops = _of_kind(objs, CardType.SCP2_OPERATION)
        if not ops:
            return None
        iid = scp2.insurgency_id(state)
        ir = scp2.ensure_scp2_state(state, iid) if iid else {}
        ...
        if len(scp2.hand_ids(state, pid)) < 2:
            audit = find("audit")
            if audit:
                return audit
    ...
    def _pick_foundation_activation(self, state, pid, r):
        ...
        thin_hand = len(scp2.hand_ids(state, pid)) < 3
        iid = scp2.insurgency_id(state)
        ir = scp2.ensure_scp2_state(state, iid) if iid else {}
        opp_rig = len(ir.get("rig", [])) if ir else 0
        for o in assets:
            cd = o.card_def
            if not callable(getattr(cd, "scp2_ability", None)):
                continue
            text = (cd.text or "").lower()
            cost = int(getattr(cd, "scp2_ability_cost", 0) or 0)
            if r["credits"] < cost:
                continue
    ...
    def _insurgency_action(self, game, pid, state, r):
        objs = _hand_objs(state, pid)
        fid = scp2.foundation_id(state)
        fr = scp2.ensure_scp2_state(state, fid) if fid else None
        ...
        if fr:
            for cell in fr["cells"]:
                ...
                adv = int(getattr(a.state, "scp2_advancement", 0)) if a else 0
        ...
        for (cell, adv, n_layers) in targets:
            if n_layers == 0:
                ok, _m, evs = scp2.infiltrate(game, pid, ("cell", cell["id"]))
                if ok:
                    return True, evs

        # 2. Build the rig — broaden breaker coverage.
        op_cards = _of_kind(objs, CardType.SCP2_OPERATIVE)
        rig_types = {getattr(o.card_def, "scp2_breaks", None)
                     for o in _installed(state, r["rig"], "operative")}
        if op_cards and (len(r["rig"]) < 3 or len(rig_types) < 2):
            missing = [o for o in op_cards if getattr(o.card_def, "scp2_breaks", None) not in rig_types]
            pick = min(missing or op_cards, key=_cost)
            if _affordable(pick, r):
                ok, _m, evs = scp2.play_card(game, pid, pick.id)
                if ok:
                    return True, evs
        ...
        if r["credits"] < 4:
            ...
            if econ_id:
                ok, _m, evs = scp2.activate_ability(game, pid, econ_id)
                if ok:
                    return True, evs
            tool_cards = _of_kind(objs, CardType.SCP2_TOOL)
            if tool_cards:
        ...
        events_in_hand = _of_kind(objs, CardType.SCP2_EVENT)
        ...
        if breach_events and fr and fr["total_breach"] >= 2:
            ...
            if _affordable(be, r):
                ok, _m, evs = scp2.play_card(game, pid, be.id)
                if ok:
                    return True, evs
        ...
        if events_in_hand:
            ...
            if _affordable(ec, r):
                ok, _m, evs = scp2.play_card(game, pid, ec.id)
                if ok:
                    return True, evs
    ...
    def _pick_insurgency_econ_activation(self, state, r):
        for o in _installed(state, r["rig"], "tool"):
            ...
            if "gain" in text and "cell" in text:
                if r["credits"] >= int(getattr(cd, "scp2_ability_cost", 0) or 0):
                    return o.id
        return None


class DispatchSCP2AIAdapter:
    """Maps player_id → SCP2AIAdapter, for per-seat difficulty in tournaments/tests."""
