...
class ActionType(str, Enum):
    ...
    # SCP (asymmetric Foundation vs Chaos Insurgency) action types
    SCP_GAIN = "SCP_GAIN"
    SCP_DRAW = "SCP_DRAW"
    SCP_PLAY = "SCP_PLAY"
    SCP_ADVANCE = "SCP_ADVANCE"
    SCP_CONTAIN = "SCP_CONTAIN"
    SCP_INFILTRATE = "SCP_INFILTRATE"
    SCP_ACTIVATE = "SCP_ACTIVATE"
...
class PlayerActionRequest(BaseModel):
    ...
    cell_id: Optional[int] = Field(default=None, description="SCP containment cell id for install/advance/contain (None = new cell)")
    scp_target: Optional[list[str]] = Field(default=None, description="SCP target tuple: ['cell','3'] or ['central','research'] for layer install / infiltrate")
...
class GameStateResponse(BaseModel):
    ...
    scp: Optional[dict] = Field(default=None, description="Nested scp engine state (factions/cells/rig/banks/hand, viewer-redacted)")
    # Game log
