"""Auto-generated interceptor verification for custom/star_wars (SWR). See /test-interceptors.

Catches the "depths trap" — interceptor wired but effect_fn returns [] silently.

SWR underwent a "slice-11 median-lift" retrofit (2026-05-19, see header in
src/cards/custom/star_wars.py:232). A subset of cards with setup_interceptors
now point at _swr_s11_*_setup helpers that emit a multi-event payload
(SCRY/SURVEIL + a self-bonus + an asymmetric-event against opponents).
The remaining cards retain pre-retrofit setup functions, many of which
...
emit through `game.emit`, because slice-11's effect_fn emits non-MTG
...
Run: PYTHONPATH=. python tests/test_star_wars_interceptors.py
"""
...
spec = importlib.util.spec_from_file_location(
    "star_wars_for_interceptor_tests",
    str(PROJECT_ROOT / "src/cards/custom/star_wars.py"),
)
_sw = importlib.util.module_from_spec(spec)
sys.modules["star_wars_for_interceptor_tests"] = _sw
spec.loader.exec_module(_sw)

STAR_WARS_CARDS = _sw.STAR_WARS_CARDS
...
SKIPPED_CARDS: dict[str, str] = {
    ...
    "Rebel Pilot":         "vanilla / simple creature (no triggered ability)",
    "Stormtrooper":        "vanilla 2/1 (no triggered ability)",
    # Modal / target-required cards — effect_fn returns [] by design until
    # the engine fills in target_chosen for these.
    "Sith Lightning":      "target-required; effect_fn=[] until target chosen",
    "Force Choke":         "target-required; effect_fn=[] until target chosen",
    "Force Push":          "target-required; effect_fn=[] until target chosen",
    "Jedi Mind Trick":     "target-required; effect_fn=[] until target chosen",
}
...
def _select_cards(limit: int = 150) -> list[tuple[str, object]]:
    ...
    Prioritise slice-11-retrofitted setups (most coverage value), then fall
    through to the rest. Cap at `limit` to keep runtime sane.
    """
    s11 = []
    other = []
    for name, cd in STAR_WARS_CARDS.items():
        ...
        if fn_name == "equipment_setup":
            continue
        if fn_name.startswith("_swr_s11_"):
            s11.append((name, cd))
        else:
...
_SELECTED = _select_cards(limit=150)
...
if __name__ == "__main__":
    ...
    print()
    print("=" * 64)
    print(" /test-interceptors — custom/star_wars (SWR)")
    print("=" * 64)
    print(f"  cards in pool:    {len(STAR_WARS_CARDS)}")
