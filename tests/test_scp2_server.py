"""Server integration + fog-of-war test for scp2.

Drives a human-vs-bot scp2 match through the real server path: create_match →
SCP2ModeAdapter.setup_game → get_client_state → handle_action. Asserts:
  1. The match creates a 2-player scp2 session; the human is the Foundation (seat 0).
  2. get_client_state serializes a `scp2` payload with me/opponent + the viewer's own hand.
  ...
  4. A human SCP2_PLAY action succeeds, and SCP2_END_TURN drives the AI's turn without error.

Run: HYPERDRAFT_STRICT=1 PYTHONPATH=. python3 -m pytest tests/test_scp2_server.py -q
"""
...
def _first_facedown_card(hand):
    ...
    for c in hand:
        if c.get("kind") in ("SCP2_ANOMALY", "SCP2_ASSET"):
            return c
    return None


def test_scp2_match_setup_and_fog_of_war():
    async def _run():
        response = await create_match(
            request=CreateMatchRequest(
                mode="human_vs_bot", game_mode="scp2", ai_difficulty="medium",
        ...
        assert session.game.state.game_mode == "scp2"
        ...
        assert st.scp2 is not None, "scp2 state must serialize"
        me = st.scp2["me"]
        assert me["faction"] == "foundation", "human (seat 0) is the Foundation"
        assert st.scp2["your_turn"] is True, "Foundation acts first"
        assert me["hand"] is not None and len(me["hand"]) >= 5, "own hand is revealed"
        assert st.scp2["opponent"]["hand"] is None, "opponent hand is hidden"
        assert st.scp2["opponent"]["hand_count"] >= 5
        ...
        ok, msg = await session.handle_action(PlayerActionRequest(
            action_type="SCP2_PLAY", player_id=human_id, card_id=card["id"],
        ))
        assert ok, f"SCP2_PLAY failed: {msg}"

        # --- 3. FOG OF WAR: the Insurgency viewer must not see the face-down identity
        ai_view = session.get_client_state(ai_id)
        assert ai_view.scp2 is not None
        blob = json.dumps(ai_view.scp2)
        ...
        opp = ai_view.scp2["opponent"]
        assert opp["faction"] == "foundation"
        if card["kind"] == "SCP2_ANOMALY":
            cells = opp.get("cells", [])
        ...
        ok, msg = await session.handle_action(PlayerActionRequest(
            action_type="SCP2_END_TURN", player_id=human_id,
        ))
        assert ok, f"SCP2_END_TURN failed: {msg}"
        ...
        assert st2.scp2 is not None
        if not st2.scp2["game_over"]:
            assert st2.scp2["your_turn"] is True, "turn returns to the Foundation"
            assert st2.scp2["me"]["ap"] == 4, "AP refreshed for the new turn"
...
def test_scp2_advance_and_credits_flow():
    """A Foundation can gain Funding and advance an installed anomaly via the server path."""
    async def _run():
        response = await create_match(
            request=CreateMatchRequest(
                mode="human_vs_bot", game_mode="scp2", ai_difficulty="easy",
                player_name="TestHuman",
            ),
            background_tasks=BackgroundTasks(),
        )
        ...
        me = session.get_client_state(human_id).scp2["me"]
        anomaly = next((c for c in me["hand"] if c.get("kind") == "SCP2_ANOMALY"), None)
        if anomaly is None:
            await session_manager.remove_session(response.match_id)
            return  # rare: no anomaly in opening hand; nothing to advance
        await session.handle_action(PlayerActionRequest(
            action_type="SCP2_PLAY", player_id=human_id, card_id=anomaly["id"]))
        # Find the installed anomaly's object id from my cells.
        cells = session.get_client_state(human_id).scp2["me"]["cells"]
        ...
        ok, msg = await session.handle_action(PlayerActionRequest(
            action_type="SCP2_ADVANCE", player_id=human_id, anomaly_id=anomaly["id"]))
        assert ok, f"advance failed: {msg}"
        cells2 = session.get_client_state(human_id).scp2["me"]["cells"]
