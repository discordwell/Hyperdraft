#!/usr/bin/env python3
"""
Frierenrift card-art pipeline (ported from the Dwellhome mall image flow).
...
Outputs:
  assets/card_art/custom/frierenrift/<card_name_snake_case>.png
"""
...
OUT_DIR = PROJECT_ROOT / "assets" / "card_art" / "custom" / "frierenrift"
...
def generate_openai_image(prompt: str, cfg: OpenAiImageConfig) -> bytes:
    ...
    for current in providers:
        try:
            ...
            last_error = exc
            if len(providers) > 1:
                print(f"[frierenrift-art] provider {current} failed; trying next: {exc}")
    if last_error:
        raise last_error
...
def card_faction_style(factions: set[str]) -> str:
    if factions == {"Frieren"}:
        return (
            "Focus on disciplined continental spellcraft, ancient elven precision, "
            "aura analysis motifs, and elegant azure-silver-verdant magical palettes."
        )
    if factions == {"Macht"}:
        return (
            "Focus on demonic golden curses, imperial menace, ruthless battlefield control, "
            "and ember-gold-obsidian palettes with sharp magical geometry."
        )
    return (
        "Show ideological tension between refined elven magic and oppressive golden-curse demon arts."
    )
...
def build_card_prompt(spec: CardArtSpec) -> str:
    ...
    details = [
        "High-end fantasy card illustration for the game Frierenrift.",
...
def build_prompt_pack(specs: list[CardArtSpec], image_size: str, quality: str, force: bool) -> dict[str, Any]:
    ...
    for spec in specs:
        ...
        entries.append(
            {
                "id": f"frierenrift-{slugify(spec.card.name)}",
                "kind": "card-art",
                ...
                "target_public_path": f"/api/card-art/custom/frierenrift/{filename}",
                "image_options": {
                    "size": image_size,
                    "quality": quality,
                    "output_format": "png",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate high-quality Frierenrift card art.")
    parser.add_argument(
        "--mode",
