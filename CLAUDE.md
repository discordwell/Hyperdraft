...
- A game mechanic can be dead at **three** levels, and each has its own check: (1) **effect** dead — `effect_fn` returns `[]` / writes a field the engine never reads → `/test-interceptors` (+ its engine-native dead-field census); (2) **mechanism** dead — a correct effect that **no engine path calls** (an `apply_<x>` win-condition hook with zero production callers; the SCP wurm/rift/leyline/spark bugs, 2026-05-29) → grep every `apply_*`/`_fire_*` for a real caller, and instrument the signature progress-counter to prove it moves in self-play; (3) **AI** dead — effect + mechanism work but the heuristic never triggers it → `/card-fire-debug`. A counter stuck at 0 across a tournament means level 2 or 3, not "balanced low."
- **Disprove a balance/AI hypothesis cheaply before committing to an expensive fix.** A runtime probe (inject the proposed change at game-setup time and measure the outcome over a few games) costs minutes; a multi-file rebalance costs hours and can be wasted on the wrong root cause. The SCP FBN-strain dig disproved "decks too fast" then "deck-balance (hazard/suppression)" with throwaway probes — both wrong — before finding the real cause (dead win-condition substrate). A 140-card hazard rebalance would have fixed nothing.
