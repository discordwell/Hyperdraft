/**
 * SCP2GameView — SCP: SECURE / CONTAIN / SUBVERT (asymmetric Foundation vs Chaos Insurgency).
 *
 ...
 * /game/:matchId/scp2 link works; reads the viewer-redacted state via useSCP2Game.
 */
...
function CardChip({ card, onClick, selected }: { card: SCP2Card; onClick?: () => void; selected?: boolean }) {
  if (card.hidden) {
    return (
  ...
  const stat =
    card.kind === 'SCP2_ANOMALY' ? `${card.trap ? 'TRAP ' : ''}${card.threshold}/${card.value}`
    : card.kind === 'SCP2_LAYER' ? `${card.ltype?.[0]?.toUpperCase()} ${card.strength}/${card.rez}`
    : card.kind === 'SCP2_OPERATIVE' ? `${card.breaks?.[0]?.toUpperCase()} p${card.power}+${card.boost}`
...
function CellView({
  ...
  cell: SCP2Cell; mine: boolean; canAct: boolean;
...
function SeatHeader({ seat, label }: { seat: SCP2Seat; label: string }) {
...
function SeatBoard({
  ...
  seat: SCP2Seat; mine: boolean; canAct: boolean;
  onAdvance?: (anomalyId: string) => void;
  onContain?: (anomalyId: string) => void;
  onInfiltrate?: (cellId: number) => void;
}) {
...
export function SCP2GameView() {
  ...
  const { state, dispatch, isConnected } = useSCP2Game();
