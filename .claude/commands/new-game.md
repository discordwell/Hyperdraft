...
**Stage 7.5 (per-card effect verification)** is critical for first sets — a freshly-built engine + freshly-generated cards have not been wet-tested together, so the "interceptor wired but effect_fn returns []" failure mode is at maximum prevalence. Do NOT skip 7.5 to "save time" on the first set; the tournament loop in Stage 8 produces meaningless balance data when half the cards are silent-failure. Read `/new-set`'s Stage 7.5 spec carefully — including its strict text-vs-events gate (run `/test-interceptors` under `HYPERDRAFT_STRICT=1 HYPERDRAFT_STRICT_STACK=1`, assert TEXT-MATCHING events) and the ⛔ ban on slice-N median-lift / `_<set>_s<N>_*` / thin-bust stubs. **Never wire a card to an effect that doesn't match its printed text, and never emit SCRY/SURVEIL/MILL/LIFE_CHANGE as generic depth filler to inflate a quality metric.** A keyword/stat-only card stays vanilla. This is how ~16 sets shipped broken; a first set must not repeat it. The set is not done until Stage 9c's text-vs-events gate passes. **Also run Stage 7.5c (fire verification)** on the deck's signature/payoff cards — a freshly-built AI adapter is where "the effect works but the AI never uses it" is most likely, and it's invisible to the text-vs-events gate. A first set's marquee cards must be shown to actually fire in self-play (`/card-fire-debug` or instrumented activation event), not just to be in-band.
...
| `completeness` | `50`. | Raise to 60–70 if Stage 7.5 verified ≥80% of cards emit **text-matching** events under strict `/test-interceptors` (NOT merely "some event fired" — a set inflated with info-pulse stubs does not qualify) AND the AI has at least 3 difficulty tiers. Never raise above 75 on a fresh build — even working engines need wet-test miles before they earn the higher tier. |
