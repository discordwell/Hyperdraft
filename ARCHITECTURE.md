...
`src/engine/scp.py` is the only **asymmetric** engine, modeled on Netrunner. One player is the **Foundation** (build & contain — owns the primary win/lose-con); the other is the **Chaos Insurgency** (infiltrate & subvert — wins through conflict). There is **no self-inflicted loss** — each faction's win is the other's loss, arbitrated by the single `check_scp_win`. The Foundation installs anomalies **face-down** and advances them over turns (a telegraphed "heat" counter) to lock them for **Containment** points (win at 6), defended by face-down **layer** stacks on its cells + central servers (HQ/Research/Archives). The Insurgency builds a **rig** of breakers and runs those sites (rez → break subroutine → access) to **free** anomalies for **Liberation** points (win at 7) or push **Total Breach** to catastrophe (win at 24) — or win by **Foundation collapse** if it looses so many anomalies that the Foundation can no longer reach Containment 6 (a decisive resolution of the old mutual-exhaustion stall; guarded by an Insurgency-hand≥2 check so a live soft-kill window isn't preempted). `src/engine/scp_turn.py:SCPTurnManager` is strict alternation, both draw 1, **no breach tick** (the old engine's self-damage is gone).

Three pillars of the asymmetry that were dead/autopilot in the first cut are now live (each gated by a self-play *fire* test, not just an effect test): **central access** (HQ trashes a Foundation hand card · Research mills 2 · Archives draws 1; the InsurgencyAI grinds an undefended central only when genuinely *walled* — `SCP_SABOTAGE` event); the **rez/break mini-game** (the engine keeps greedy *defaults*, but the AI passes smart policies into `infiltrate` — `foundation_rez_policy` rezzes *to stop, not decorate*; `insurgency_break_policy` *eats* cheap subroutines to conserve Cells — `src/ai/scp_adapter.py`; the server supplies the bot Foundation's rez policy on a human-Insurgency run); and the **burnout** soft-kill (Foundation op *Enhanced Interrogation* deals damage = the Insurgency's `exposed` count, a live minority win). Each of the four decks now runs its own **archetype-aligned identity** (passive on the player record): Site-19 Command (max hand) / **Overseer Council** (`damage_bonus` +1 vs an exposed Insurgency) for Foundation; Black Queen Cell (steal) / **Sarkic Cult** (`breach_event_bonus` +1 — makes the breach axis a *real* primary win, not a borrowed liberation engine) / **Black Lodge Cell** (`mill_bonus` +1 — the *denial* archetype: mill the Foundation's deck + free anomalies until it can't reach Containment 6 and loses by **collapse**, the third Insurgency deck) for Insurgency. The Foundation answers the breach clock with **Containment Sweep** (−5 Total Breach; `reduce_breach`), and answers *denial* with **Containment Recovery** (`recover_anomaly` — re-secure a milled/freed anomaly from the discard onto a cell **1 advance from locking**, so it locks ahead of the re-free; re-securing part-way just donates a re-free — measured 22% vs 49% near-locked; **cost 3** so the premium effect is a *choice* not a +5pt auto-include staple — it taxes a lean splash but not the funded build-around). That card is the engine of the **third Foundation archetype "Reliquary Recontainment"** (grind/control; hard-counters Black Lodge denial ~57-62%, loses to pure steal-tempo — a clean RPS; reuses Site-19). It's also a **freely splashable** answer (+~3pt situational tech in any Foundation deck vs mill/steal). Total Breach catastrophe is **24** (10→14→16→24 — raised once the Sarkic engine made breach genuinely explosive). The **burnout** axis later got a dedicated deck — **"Black-Site Interrogation"** (Overseer Council): it ignores the containment race and wins by flatlining the Insurgency's hand (Mobile Task Force *proactive* expose + sensors stacking non-resetting exposure + damage-traps → *Enhanced Interrogation* deals damage = total exposure). Because hand-starvation also trips below the **hand≥2 gate** that the `foundation_collapse` win requires, it is near-immune to collapse — and is the only Foundation build to beat the **Discordwell's Gambit** boss in fog-honest LLM-pilot play (every *containment* Foundation, incl. the Dr. Light boss, loses to the Gambit's hand-lock+mill collapse under skilled play). **Six** Foundation × four Insurgency archetypes balance to **~50% Foundation** (within the 35-65% guard), all five win axes live (containment / liberation / collapse / burnout / breach), 0 stalls. AI difficulty is a real ladder on the Insurgency seat (easy only takes undefended anomalies; hard strikes earlier + pushes breach proactively); medium is the tuned-balanced bot.
...
1. ~~**`game_mode == "..."` string branching**~~ **Partially resolved.** The majority of scattered mode checks in the engine and server layers now route through `src/engine/mode_adapter.py:GameModeAdapter` and its concrete subclasses (`MTGModeAdapter`, `HearthstoneModeAdapter`, `PokemonModeAdapter`, `YugiohModeAdapter`), dispatched via `get_mode_adapter(game_mode)`. Every behavioral hook (hand-size cap, board cap, armor/hero damage routing, fatigue/mill, weapon cleanup, zone-leave reset, Divine Shield/Immune registration, subsystem factories, zone creation, starting-hand setup, AI-player registration, battlefield-serialization filtering, game-log surfacing) maps to an explicit adapter method. Remaining checks are: mode-dispatched match-setup paths in `src/server/routes/match.py` (deck selection wizards), per-mode main loops in `src/server/session.py` (`_run_{hs,pkm,ygo,mtg}_game_loop`), per-mode zone-serialization blocks in `session.py.get_client_state` (PKM active/bench/stadium, YGO monster/spell-trap/field zones — each is a distinct code path, not a condition on otherwise-generic logic), a turn-limit literal in `src/server/routes/bot_game.py`, and defensive HS-only `game_mode == "hearthstone"` guards in `src/cards/hearthstone/*` card scripts that only ever load in HS mode. These are all cases where the branch corresponds to an entire subpath, not a pluggable behavior.
